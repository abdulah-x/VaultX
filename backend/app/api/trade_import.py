...
from services.analytics.trade_stats import average_holding_times, fee_breakdown, win_rate
...
def _commission_prices(db: Session, trades) -> Dict[str, Decimal]:
    """USD price for each asset fees were charged in, in one query.

    Stablecoins are handled inside `fee_breakdown` and don't need a row here.
    """
    assets = {t.commission_asset for t in trades if t.commission_asset}
    if not assets:
        return {}
    rows = (
...
async def get_trade_analysis(
    ...
    try:
        ...
        # The three questions this endpoint exists to answer, all over the same
        # filtered trade set: how long you hold, how often you're right, and what
        # it costs you.
        analysis["holding_time"] = average_holding_times(trades)
        analysis["win_rate"] = win_rate(trades)
        analysis["fees"] = fee_breakdown(trades, _commission_prices(db, trades))
...
async def get_fee_breakdown(
    ...
    This is the fee section of `/trades/analysis` on its own, for callers that
    only want fees over a longer window than the analysis default. Both share
    `services.analytics.trade_stats.fee_breakdown`, so the numbers can't drift.
    """
    try:
        start_date = datetime.utcnow() - timedelta(days=days)
        trades = (
        ...
        return stringify_decimals({
            ...
            "fees": fee_breakdown(trades, _commission_prices(db, trades)),
        })
